...
class SatNet{
    ...
    void insertPrivate(const Sat& satellite, Sat*& ptr);
    int max(int x, int y);
    int height(Sat* ptr);
    void updateHeight(Sat* ptr);
    Sat* rightRotate(Sat* ptr);
    Sat* leftRotate(Sat* ptr);
    void balanceNode(Sat*& ptr);
    void inOrder(Sat* aNode) const;
    void deleteSubTree(Sat* ptr);
    void removePrivate(Sat*& ptr, int id);
    Sat* findMin(Sat* ptr);
    void setStatePrivate(int id, STATE state, Sat* ptr, bool& found);
    void inOrderTraverse(INCLIN degree, Sat* ptr, int& count) const;
    void findSatellitePrivate(int id, Sat* ptr, bool& found) const;
    void removeDeorbitedPrivate(Sat* ptr);
    Sat* copyTree(const Sat* ptr);
    bool isBalanced(Sat* ptr);
    bool isBST();
    void isBSTPrivate(Sat* ptr, bool& balanced);
};
